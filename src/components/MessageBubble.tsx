...
const MessageBubble: React.FC<MessageBubbleProps> = ({ message }) => {
  ...
  const bubbleClasses = isUser
    ...
    : isError
    ? 'bg-red-600 text-white self-start rounded-r-xl rounded-tl-xl shadow-md'
    : isSystem
    ? 'bg-amber-500/10 text-amber-600 self-center text-xs italic rounded-lg border border-amber-500/20 px-3 py-1.5'
  ...
  const formatText = (text: string): string => {
    ...
    html = html.replace(/PRODUCT_LINK\[([^|]+)\|([^\]]+)\]/g, (match, handle, title) => {
        ...
        return `<a href="${productUrl}" target="_blank" rel="noopener noreferrer" class="text-primary-light hover:underline font-medium">${title}</a>`;
    });
    ...
    html = blocks.map(block => {
        if (block.match(/^<\/?(ul|ol|li|strong|em|a)>$/i) || block.startsWith('<ul') || block.startsWith('<ol') || block.startsWith('<a')) {
            return block;
        }
